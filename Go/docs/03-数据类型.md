...
结构体标签是附加在结构体字段上的元数据，使用反引号 `` `...` `` 包裹。标签主要用于序列化、验证、ORM 映射等场景。
...
type User struct {
    ...
    Name     string `json:"name" validate:"required"`  // JSON 和验证标签
    Email    string `json:"email" validate:"email"`   // 验证标签
    Password string `json:"-"`                         // "-" 表示序列化时忽略
    Age      int    `json:"age,omitempty"`             // omitempty 表示零值时忽略
}
```

**常见标签类型**：

1. **JSON 标签**：用于 JSON 序列化/反序列化
```go
type Person struct {
    FirstName string `json:"first_name"`  // JSON 中使用 "first_name"
    LastName  string `json:"last_name"`   // JSON 中使用 "last_name"
...
// 输出: {"first_name":"John","last_name":"Doe","age":30}
```

2. **YAML 标签**：用于 YAML 序列化/反序列化
```go
type Config struct {
...
type User struct {
    ...
    Email    string `db:"email" gorm:"index"`
}
```

4. **验证标签**：用于数据验证（需要验证库支持）
```go
...
1. 标签必须使用反引号 `` `...` ``
2. 多个标签用空格分隔：`` `json:"name" db:"user_name"` ``
3. 标签值通常使用 `key:"value"` 格式
4. 特殊值：
   - `"-"`：表示忽略该字段
   - `"omitempty"`：零值时忽略（JSON/YAML）
   - `",omitempty"`：与前面的标签组合使用
...
**与其他语言对比**：
- **C++**: 使用宏或模板元编程实现类似功能
- **Rust**: 使用过程宏（procedural macros）如 `serde`
- **Java**: 使用注解（Annotations）如 `@JsonProperty`
